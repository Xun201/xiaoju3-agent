...
def handle_message(source, user_id, group_id, message):
    ...
    if isinstance(message, list):
        message = "".join([seg.get("data", {}).get("text", "") for seg in message if seg.get("type") == "text"])
            # === 🛡️ 指令菜单 ===
    if message.strip() in ["/help", "菜单", "帮助", "指令"]:
    ...
    if message.startswith("/coder_auth"):
        auth_code = message.replace("/coder_auth", "").strip()
        if auth_code == "coder_default":
            permission_manager.current_level = "Lv.3"
            return "✅ 已激活代码编写者权限（Lv.3）。你可以读写代码和插件。"
        else:
            return "❌ 认证码错误。"
# 保留这段，让消息顺利进入权限验证
    # === 🛡️ 全局动态密码激活（Lv.4） ===
    if message.startswith("/auth_xun"):
        auth_code = message.replace("/auth_xun", "").strip()
        success, msg = permission_manager.verify_xun(auth_code, source="QQ/网页")
        
        # 🎵 彩蛋：激活成功时，通知电脑播放音效
        if success:
            try:
                # ⚠️ 确保这里的IP是你电脑的 Tailscale IP
                requests.get("http://100.97.222.37:5003/play", timeout=3)
                print("🎵 已成功通知电脑播放开机音效！")
            except Exception as e:
                print(f"⚠️ 通知电脑播放失败（可能是防火墙或IP错误）: {e}")
                
        return msg

    # === 🛡️ 发送图片指令 ===
    if message.startswith("/send_image"):
        if permission_manager.current_level not in ["Lv.3", "Lv.4"]:
            return "❌ 权限不足，请先发送 /coder_auth coder_default 升级权限。"
        img_path = message.replace("/send_image", "").strip()
        if not os.path.exists(img_path):
            return f"❌ 图片不存在：{img_path}"
        if not img_path.startswith("/home/orangepi/xiaoju3-agent"):
            return "❌ 只能发送项目工作区内的图片。"
        
        try:
            import base64
            from config import NAPCAT_API_URL, NAPCAT_TOKEN
            ...
            resp = requests.post(f"{NAPCAT_API_URL}/{api_endpoint}", json=payload, headers={"Authorization": f"Bearer {NAPCAT_TOKEN}"}, timeout=20).json()
    ...
    # 1. 收集图片表情包（拦截图片消息）
    # 只有真正的图片消息才处理：包含图片CQ码，且不包含@
    if "[CQ:image" in message and "[CQ:at" not in message:
    ...
    if source == 'web':
        ...
        res = smart_ask(messages_web, message)
        ...
        res = smart_ask(messages_qq, message)
        reply = res[0] if isinstance(res, tuple) else res
        if not reply:
            reply = "抱歉，我脑袋突然卡壳了，请再说一遍吧。"
        ...
        if reply:
            try:
                ...
                requests.post(f"{NAPCAT_API_URL}/{api_endpoint}", json=payload, 
                              timeout=10, headers={"Authorization": f"Bearer {NAPCAT_TOKEN}"})
            except Exception as e:
                print(f"❌ 发送QQ消息失败: {e}")
...
if __name__ == '__main__':
    print("🌐 小橘3号网页控制台已启动！")
    print("💡 网页访问：http://192.168.31.82:5002")
